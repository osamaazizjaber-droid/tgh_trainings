	...
	_createClass(ImageModule, [{
		...
		value: function render(part, options) {
			...
			if (!tagValue) {
				...
				// If it's a Buffer or Uint8Array, treat as raw image data (not {rId,sizePixel})
				if (tagValue.rId !== undefined && tagValue.sizePixel !== undefined) {
					return this.getRenderedPart(part, tagValue.rId, tagValue.sizePixel);
				}
				// Fall through to treat as raw image buffer
			}
